...
const sideMeta = {}
for (const [jname, side] of sideJons) {
...
for (const [jname, side] of sideJons) {
  ...
  const targetDir = path.join(minioBase, productId, serial, sideDir)
  const jsonDest = path.join(targetDir, describeName)

  if (dryRun) {
    ok(`[dry-run] ${jname} → ${jsonDest}`)
  } else {
    await copyFile(jpath, jsonDest)
    ok(`${jname} → ${jsonDest}`)
  }

  // 缺陷图片: img/deepiresults/<产品>/<SN>/<sideDir>/**
  const imgSideDir = path.join(imgRoot, productId, serial, sideDir)
