<template>
  <div class="page">
    ...
    根据日期范围查询 history_data_list_datetime 中的历史数据
    ...
    <el-card class="form-card">
      <el-form :model="store" label-width="120px">
        ...
        <el-form-item label="开始日期">
          <el-input
            v-model="store.rangeStart"
            placeholder="例如: 20260805"
            clearable
          />
        </el-form-item>
...
import { useGetHistoryDataRangeStore } from '../stores/dataMaintenance'

const store = useGetHistoryDataRangeStore()
...
const handleQuery = async () => {
  // 验证表单
  if (!store.rangeStart || !store.rangeEnd || !store.serverUrl) {
    ElMessage({
      message: '请填写日期范围和服务器地址',
      type: 'warning',
      offset: 80,
    })
    return
  ...
  try {
    ...
    const output = {
      ...
      op_mode: 'all',
      operation: 'range_get',
      range_start: store.rangeStart,
      range_end: store.rangeEnd,
    }
