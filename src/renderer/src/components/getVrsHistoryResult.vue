<template>
  <div class="page">
    ...
    <el-card class="form-card">
      <el-form :model="getVrsHistoryStore" label-width="120px">
        <el-form-item label="服务器地址">
          <el-input
            v-model="getVrsHistoryStore.serverUrl"
    ...
    <el-card v-if="getVrsHistoryStore.responseResult" class="result-card">
      <template #header>
        <div class="card-header">
          <span>接口返回结果</span>
...
import { useGetVrsHistoryStore } from "../stores/dataMaintenance";

const getVrsHistoryStore = useGetVrsHistoryStore();
...
const handleQuery = async () => {
  // 验证表单
  if (!getVrsHistoryStore.sn || !getVrsHistoryStore.serverUrl) {
    ElMessage({
      message: "请填写SN和服务器地址",
      type: "warning",
      offset: 80,
  ...
  try {
    ...
    getVrsHistoryStore.setResponseResult(`请求失败: ${error}`);
    ElMessage({
      message: "查询失败",
      type: "error",
      offset: 80,
